...
public:
	...
	void SetMaterialType(UINT nType) { m_nType |= nType; } // setaldedo랑 다름
...
public:
	...
	void SetRadius(float fBoundRadius) { m_fRadius = fBoundRadius; }
	...
	virtual float CalculateDistance(XMFLOAT3 xmf3Pos);
...
public:
	bool m_BulShot = false;

	XMFLOAT3 m_xmf3BulPosition;

	virtual void Animate(float fTimeElapsed, XMFLOAT4X4* pxmf4x4Parent = NULL);

	void SetBulPosition(XMFLOAT3 xmf3BulPosition) { m_xmf3BulPosition = xmf3BulPosition; }
	void SetBulShot(bool bulshot) { m_BulShot = bulshot; }

	XMFLOAT3 GetBulPosition() { return m_xmf3BulPosition; }
	bool GetBulShot() { return m_BulShot; }
