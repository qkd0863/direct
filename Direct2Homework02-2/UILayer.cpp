...
void UILayer::Render(UINT nFrame)
{
    ...
    ID2D1SolidColorBrush* pd2dBulletBrush = CreateBrush(D2D1::ColorF(D2D1::ColorF::Gold, 1.0f));
    ID2D1SolidColorBrush* pd2dBulletLineBrush = CreateBrush(D2D1::ColorF(D2D1::ColorF::Black , 1.0f));
    ID2D1SolidColorBrush* pd2dHpBrush = CreateBrush(D2D1::ColorF(D2D1::ColorF::DarkRed , 0.7f));
    ID2D1SolidColorBrush* pd2MinimapLineBrush = CreateBrush(D2D1::ColorF(D2D1::ColorF::LightGreen , 1.0f));
    ID2D1SolidColorBrush* pd2EnemyBrush = CreateBrush(D2D1::ColorF(D2D1::ColorF::DarkRed , 1.0f));
    D2D1_POINT_2F d2dcenter;
    d2dcenter.x = FRAME_BUFFER_WIDTH * 0.85f;
    d2dcenter.y = FRAME_BUFFER_HEIGHT * 0.9f;
    D2D1_POINT_2F d2dPlayerCenter;
    d2dPlayerCenter.x = FRAME_BUFFER_WIDTH * 0.1f;
    d2dPlayerCenter.y = FRAME_BUFFER_HEIGHT * 0.1f;
    D2D1_ELLIPSE d2dBulletHead = D2D1::Ellipse(d2dcenter, 13.0f, 15.0f);
    D2D1_ELLIPSE d2dPlayerPos = D2D1::Ellipse(d2dPlayerCenter, 5.0f, 5.0f);
    D2D1_RECT_F d2dBulletBody1 = D2D1::RectF(FRAME_BUFFER_WIDTH * 0.85f - 15.0f, FRAME_BUFFER_HEIGHT * 0.9f , FRAME_BUFFER_WIDTH * 0.85f + 15.0f, FRAME_BUFFER_HEIGHT * 0.965f);
    D2D1_RECT_F d2dBulletBody2 = D2D1::RectF(FRAME_BUFFER_WIDTH * 0.85f - 16.0f, FRAME_BUFFER_HEIGHT * 0.97f , FRAME_BUFFER_WIDTH * 0.85f + 16.0f, FRAME_BUFFER_HEIGHT * 0.98f);
    D2D1_RECT_F d2dBulletLine1 = D2D1::RectF(FRAME_BUFFER_WIDTH * 0.85f - 15.0f, FRAME_BUFFER_HEIGHT * 0.9f , FRAME_BUFFER_WIDTH * 0.85f + 15.0f, FRAME_BUFFER_HEIGHT * 0.905f);
    D2D1_RECT_F d2dBulletLine2 = D2D1::RectF(FRAME_BUFFER_WIDTH * 0.85f - 11.0f, FRAME_BUFFER_HEIGHT * 0.965f , FRAME_BUFFER_WIDTH * 0.85f + 11.0f, FRAME_BUFFER_HEIGHT * 0.97f);
    D2D1_RECT_F d2dMinimapLineRect = D2D1::RectF(0.0f, 0.0f, FRAME_BUFFER_WIDTH * 0.2f, FRAME_BUFFER_HEIGHT * 0.2f);

    float fPlayerHpPercent = (float)m_nPlayerCurHp / (float)m_nPlayerMaxHp;
    D2D1_RECT_F d2dHpBar = D2D1::RectF(100.0f, FRAME_BUFFER_HEIGHT - 100.0f, 100.0f + (FRAME_BUFFER_WIDTH * 0.4f * fPlayerHpPercent), FRAME_BUFFER_HEIGHT - 20.0f);
    ...
    m_pd2dDeviceContext->FillEllipse(d2dBulletHead, pd2dBulletBrush);
    m_pd2dDeviceContext->FillEllipse(d2dPlayerPos, pd2dBulletBrush);
    m_pd2dDeviceContext->FillRectangle(d2dBulletBody1, pd2dBulletBrush);
    m_pd2dDeviceContext->FillRectangle(d2dBulletBody2, pd2dBulletBrush);
    m_pd2dDeviceContext->FillRectangle(d2dBulletLine1, pd2dBulletLineBrush);
    m_pd2dDeviceContext->FillRectangle(d2dBulletLine2, pd2dBulletLineBrush);
    m_pd2dDeviceContext->FillRectangle(d2dHpBar, pd2dHpBrush);

    m_pd2dDeviceContext->DrawRectangle(d2dMinimapLineRect, pd2MinimapLineBrush, 1.0f);

    for (int i = 0; i < MAX_ENEMY_COUNT; ++i)
    {
        if (m_xmf2EnemyPos[i].x != NULL_POS && m_xmf2EnemyPos[i].y != NULL_POS)
        {
            D2D1_POINT_2F d2dEnemyCenter;
            d2dEnemyCenter.x = d2dPlayerCenter.x + m_xmf2EnemyPos[i].y;
            d2dEnemyCenter.y = d2dPlayerCenter.y + m_xmf2EnemyPos[i].x;
            if (d2dEnemyCenter.x <= FRAME_BUFFER_WIDTH * 0.2f && d2dEnemyCenter.y <= FRAME_BUFFER_HEIGHT * 0.2f)
            {
                D2D1_ELLIPSE d2dEnemyPos = D2D1::Ellipse(d2dEnemyCenter, 5.0f, 5.0f);
                m_pd2dDeviceContext->FillEllipse(d2dEnemyPos, pd2EnemyBrush);
            }
        }
    }
    ...
    pd2dBulletBrush->Release();
    pd2dBulletLineBrush->Release();
    pd2dHpBrush->Release();
    pd2MinimapLineBrush->Release();
    pd2EnemyBrush->Release();
